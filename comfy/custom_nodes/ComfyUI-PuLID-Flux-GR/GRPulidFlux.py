...
def has_insightface_model(root, name):
    model_dir = os.path.join(root, "models", name)
    required_files = INSIGHTFACE_MODEL_FILES.get(name)
    if not required_files:
        return os.path.isdir(model_dir)
    return os.path.isdir(model_dir) and all(
        os.path.isfile(os.path.join(model_dir, file_name))
        for file_name in required_files
    )

def _insightface_root_from_file_path(full_path, name):
    model_dir = os.path.dirname(os.path.abspath(full_path))
    if os.path.basename(model_dir) != name:
        return None
...
def _iter_extra_config_insightface_roots():
    yaml_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "extra_model_paths.yaml"))
    ...
    for conf in config.values():
        ...
        for raw_path in value.splitlines():
            ...
            if os.path.isabs(raw_path):
                ...
                relative_path = os.path.join("..", raw_path)
                if base_path:
                    full_path = os.path.join(os.path.expandvars(os.path.expanduser(str(base_path))), relative_path)
                else:
                    full_path = os.path.join(yaml_dir, relative_path)
            roots.append(os.path.normpath(os.path.abspath(full_path)))
    return roots

def _find_insightface_root_from_registered_files(name):
    for file_name in INSIGHTFACE_MODEL_FILES.get(name, ()):
        try:
            full_path = folder_paths.get_full_path("insightface", os.path.join("models", name, file_name))
        except Exception:
            full_path = None
        if not full_path:
            continue
        root = _insightface_root_from_file_path(full_path, name)
        if root and has_insightface_model(root, name):
            return root
    return None

def resolve_insightface_model_root(name):
    roots = []
    registered_root = _find_insightface_root_from_registered_files(name)
    if registered_root:
        roots.append(registered_root)
    try:
        roots.extend(folder_paths.get_folder_paths("insightface"))
    except Exception:
        pass
    try:
        roots.extend(folder_paths.folder_names_and_paths.get("insightface", ([], set()))[0])
    except Exception:
        pass
    roots.extend(_iter_extra_config_insightface_roots())
    roots.append(INSIGHTFACE_DIR)
    ...
    for root in roots:
        ...
        if key in seen:
            continue
        seen.add(key)
        if has_insightface_model(root, name):
            print(f"GR PuLID Flux InsightFace using model root: {root}")
            logging.info("GR PuLID Flux InsightFace using model root: %s", root)
            return root

    fallback_root = os.path.normpath(roots[0]) if roots else INSIGHTFACE_DIR
    print(f"GR PuLID Flux InsightFace model not found, using root: {fallback_root}")
    logging.info("GR PuLID Flux InsightFace model not found, using root: %s", fallback_root)
    return fallback_root

def resolve_eva_clip_dir():
    for folder_name in ("clip", "text_encoders"):
        try:
            clip_file_path = folder_paths.get_full_path(folder_name, EVA_CLIP_FILE)
        except Exception:
            clip_file_path = None
        if clip_file_path:
            clip_dir = os.path.dirname(clip_file_path)
            logging.info("GR PuLID Flux EVA-CLIP using model dir: %s", clip_dir)
            return clip_dir

    logging.info("GR PuLID Flux EVA-CLIP model not found, using dir: %s", CLIP_DIR)
    return CLIP_DIR
...
class GRPulidFluxInsightFaceLoader:
    ...
    def load_insightface(self, provider):
        insightface_root = resolve_insightface_model_root("antelopev2")
        self.model = FaceAnalysis(name="antelopev2", root=insightface_root, providers=[provider + 'ExecutionProvider',]) # alternative to buffalo_l
...
class GRPulidFluxEvaClipLoader:
    ...
    def load_eva_clip(self):
        ...
        model, _, _ = create_model_and_transforms(
            ...
            cache_dir=resolve_eva_clip_dir(),
        )
