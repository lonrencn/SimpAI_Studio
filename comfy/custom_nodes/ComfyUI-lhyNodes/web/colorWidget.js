...
const AILabColorWidget_copy = {
    COLOR_CODE: (key, val) => {
        const widget = {};
        widget.y = 0;
        widget.name = key;
        widget.type = 'COLOR_CODE';
        ...
        widget.draw = function (ctx, node, widgetWidth, widgetY, height) {
            const hide = this.type !== 'COLOR_CODE' && app.canvas.ds.scale > 0.5;
            if (hide) {
                return;
            }
...
app.registerExtension({
    name: "AILab.colorWidget.copy",

    getCustomWidgets() {
        return {
            COLOR_CODE: (node, inputName, inputData) => {
                return {
                    widget: node.addCustomWidget(
                        AILabColorWidget_copy.COLOR_CODE(inputName, inputData) 
                    ),
                    minWidth: 150,
                    minHeight: 22,
                };
