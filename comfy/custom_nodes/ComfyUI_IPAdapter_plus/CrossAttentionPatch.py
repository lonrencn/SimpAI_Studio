...
class Attn2Replace:
    ...
    def remove_by_cleanup_id(self, cleanup_id):
        if cleanup_id is None:
            return
        for i in range(len(self.kwargs) - 1, -1, -1):
            if self.kwargs[i].get("_ipa_cleanup_id") == cleanup_id:
                self.kwargs.pop(i)
                self.callback.pop(i)
    ...
    def _refresh_attrs(self):
        for key in self._dynamic_attr_keys:
            if hasattr(self, key):
                delattr(self, key)
        self._dynamic_attr_keys.clear()
        ...
        for key, value in last.items():
            if hasattr(Attn2Replace, key):
                continue
            setattr(self, key, value)
            self._dynamic_attr_keys.add(key)
    ...
    def to(self, device, *args, **kwargs):
        ...
        if self.kwargs[0].get("cond") is None:
            return self
