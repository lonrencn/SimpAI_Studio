...
def resolve_insightface_model_root(name):
    ...
    for root in roots:
        ...
        if has_insightface_model(root, name):
            print(f"PuLID Flux InsightFace using model root: {root}")
            logging.info("PuLID Flux InsightFace using model root: %s", root)
            return root
    fallback_root = os.path.normpath(roots[0]) if roots else INSIGHTFACE_DIR
    print(f"PuLID Flux InsightFace model not found, using download root: {fallback_root}")
    logging.info("PuLID Flux InsightFace model not found, using download root: %s", fallback_root)
...
class PulidFluxInsightFaceLoader:
    ...
    def load_insightface(self, provider):
        name = "antelopev2"
        insightface_root = resolve_insightface_model_root(name)
        download_insightface_model("models", name, root=insightface_root)
        model = FaceAnalysis(name=name, root=insightface_root, providers=[provider + 'ExecutionProvider', ]) # alternative to buffalo_l
