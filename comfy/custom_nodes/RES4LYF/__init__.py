...
if beta57_scheduler_name not in SCHEDULER_HANDLERS:
    # 创建一个处理beta57的函数
    def beta57_handler_func(model_sampling, steps):
        from comfy.samplers import beta_scheduler
        return beta_scheduler(model_sampling, steps, alpha=0.5, beta=0.7)
    
    # 使用SchedulerHandler注册beta57调度器
    beta57_scheduler_handler = SchedulerHandler(handler=beta57_handler_func, use_ms=True)
    SCHEDULER_HANDLERS[beta57_scheduler_name] = beta57_scheduler_handler
    # 确保beta57被添加到名称列表
    if beta57_scheduler_name not in SCHEDULER_NAMES:
        SCHEDULER_NAMES.append(beta57_scheduler_name)
