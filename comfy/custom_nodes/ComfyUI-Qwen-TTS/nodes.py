...
def _qwen_tts_roots() -> List[str]:
    roots = []
    try:
        roots = folder_paths.get_folder_paths("qwen-tts") or []
    except Exception:
        roots = []

    fallback = os.path.join(folder_paths.models_dir, "qwen-tts")
    out = []
    seen = set()
    for root in list(roots) + [fallback]:
        if not root:
            continue
        try:
...
def _preferred_qwen_root(qwen_roots: List[str]) -> str:
    for root in qwen_roots:
        if root and os.path.isdir(root):
            return root
    return qwen_roots[0] if qwen_roots else os.path.join(folder_paths.models_dir, "qwen-tts")


def _find_existing_qwen_model_dir(model_id: str, qwen_roots: List[str]) -> Optional[str]:
    folder_name = model_id.split("/")[-1]
    for root in qwen_roots:
        for candidate in (os.path.join(root, folder_name), os.path.join(root, "Qwen", folder_name)):
...
def check_and_download_tokenizer():
    ...
    qwen_roots = _qwen_tts_roots()
    qwen_root = _preferred_qwen_root(qwen_roots)
    os.makedirs(qwen_root, exist_ok=True)

    # Download tokenizer (required by all models)
    tokenizer_id = "Qwen/Qwen3-TTS-Tokenizer-12Hz"
    if _find_existing_qwen_model_dir(tokenizer_id, qwen_roots):
...
def load_qwen_model(model_type: str, model_choice: str, device: str, precision: str, attention: str = "auto", unload_after: bool = False, previous_attention: str = None, custom_model_path: Optional[str] = None):
    ...
    if not found_local and not os.path.isdir(final_source):
        existing_qwen_model = _find_existing_qwen_model_dir(final_source, qwen_roots)
        if existing_qwen_model:
            found_local = existing_qwen_model
