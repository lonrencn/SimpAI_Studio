...
function simpleai_mark_user_scroll_intent() {
    window.__simpaiViewerUserScrollIntentUntil = Date.now() + 2500;
}

function simpleai_has_recent_user_scroll_intent() {
    return Date.now() < (window.__simpaiViewerUserScrollIntentUntil || 0);
}

function simpleai_reset_initial_scroll(delay = 0) {
    setTimeout(() => {
        if (window.location.hash || simpleai_has_recent_user_scroll_intent()) return;
        window.scrollTo({ top: 0, left: 0, behavior: 'auto' });
    }, delay);
}

if (!window.__simpaiViewerScrollGuardInstalled) {
    window.__simpaiViewerScrollGuardInstalled = true;
    try {
    ...
    ['pointerdown', 'mousedown', 'touchstart', 'keydown', 'wheel'].forEach((eventName) => {
        window.addEventListener(eventName, simpleai_mark_user_scroll_intent, { capture: true, passive: true });
    });

    simpleai_reset_initial_scroll(0);
    simpleai_reset_initial_scroll(80);
    simpleai_reset_initial_scroll(350);
    simpleai_reset_initial_scroll(1200);
}

window.__simpaiCanAutoViewerScroll = simpleai_has_recent_user_scroll_intent;
...
function viewer_to_bottom(delay = 100) {
    if (!simpleai_has_recent_user_scroll_intent()) return;
...
onUiLoaded(async () => {
    resized();
    simpleai_reset_initial_scroll(0);
    simpleai_reset_initial_scroll(400);
    simpleai_reset_initial_scroll(1200);
});
...
onUiLoaded(async () => {
    ...
    if (styleSelections && styleSelections.dataset.blurHandlerBound !== 'true') {
        styleSelections.dataset.blurHandlerBound = 'true';
        styleSelections.addEventListener('focusout', function () {
            setTimeout(() => {
